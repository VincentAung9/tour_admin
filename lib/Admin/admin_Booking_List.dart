      ...
      if (response.data != null) {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'Error loading bookings: ${response.errors.first.message}')),
          );
        }
      }
    } catch (e) {
    ...
    try {
      ...
      if (response.data != null) {
        ...
        setState(() {
          ...
          booking['updatedAt'] =
              updatedBooking['updatedAt'] ?? booking['updatedAt'];
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Text('ID: ${booking['id'] ?? 'N/A'}',
                    style: const TextStyle(fontWeight: FontWeight.bold)),
                Text(
                    'User: ${booking['userName'] ?? 'Unknown'} (${booking['userId'] ?? 'N/A'})'),
                Text('Email: ${booking['email'] ?? 'No email'}'),
                Text('Phone: ${booking['phoneNumber'] ?? 'No phone'}'),
                const SizedBox(height: 6),
                Text(
                    'Plan: ${booking['planName'] ?? 'N/A'} - ${booking['planPrice'] ?? 'N/A'}'),
                ...
                Text(
                    'Payment Intent ID: ${booking['paymentIntentId'] ?? 'N/A'}'),
                const SizedBox(height: 6),
                Text(
                    'Tour: ${booking['tourTitle'] ?? 'N/A'} (${booking['tourLocation'] ?? 'N/A'})'),
                Text('Tour ID: ${booking['tourId'] ?? 'N/A'}'),
                if (booking['tourImageUrl'] != null &&
                    booking['tourImageUrl'].toString().isNotEmpty)
                  Padding(
